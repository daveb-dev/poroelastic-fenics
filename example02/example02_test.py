...
bndry = MeshFunction('size_t', mesh, 2)
bndry.set_all(0)
left.mark(bndry, 28)
right.mark(bndry, 31)
front.mark(bndry, 30)
...
U = VectorElement("CG", mesh.ufl_cell(),1, 3) # displacement
V = FiniteElement("RT", mesh.ufl_cell(),1,3) # velocity
#V = FiniteElement("BDM", mesh.ufl_cell(),1) # velocity
P = FiniteElement("DG", mesh.ufl_cell(),0, 3) # pressure
...
def getBCs():
    ...
    #bcs.append(DirichletBC(W.sub(1), (0.0,0.0,0.0), bndry,26)) # flux 
...
L0 = inner(sigma(umid), sym(grad(v)))*dx  -  alpha*pmid*div(v)*dx \
    - inner(f, v)*dx - inner(t, v)*dss(26)  #+ alpha*inner(pbc*n,v)*dss(26)

# Darcy velocity
L1 =(inner(inv(K)*qmid, z) - pmid*div(z))*dx + inner(z, pbc*n)*dss(26)
