...
k_ = Function(KSpace)
k_.vector()[:] = np.random.rand(num_cell)

# similarly, phi_ = Function(KSpace)
...
s0 = Constant(1.0/M)
# s02_ = MeshFunction("double", mesh, mesh.topology().dim(),0.0)
# s02_.array()[:] = s0_[:]
...
def getBCs():
    ...
    # Left boundary
    bcs.append(DirichletBC(W.sub(0).sub(0), 0.0, bndry,5)) # No normal displacement for solid on left
    bcs.append(DirichletBC(W.sub(1), (0.0,0.0), bndry,5)) 

    
    # Right boundary
    bcs.append(DirichletBC(W.sub(0).sub(0), 0.0, bndry,3)) # No normal displacement for solid on right
    ...
    # bottom boundary
    bcs.append(DirichletBC(W.sub(0).sub(1), 0.0, bndry,2)) # No normal displacement for solid on bottom
