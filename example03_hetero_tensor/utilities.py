...
def gaussianField(range_=(0,1), size=1):
    """Generate range values between given range

    :size: Numper of samples
    :range_: given range
    :returns: numpy array of random number

    """
    # mu, sigma = 0.3, 0.1 # mean and standard deviation
    min_, max_ = range_
    values = np.random.rand(size)
    return  min_ + (values * (max_-min_)) # scaled_value

def perm(phi, tau=0.81, dp=10e-6):
    return phi**3*(dp)**2/(tau*72*(1.- phi)**2)
        



def Mc(phi,cf,cr=1.0e-12,alpha=1.0):
