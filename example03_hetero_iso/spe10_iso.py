...
import pandas as pd
from mesh_creater import  MeshCreater
...
mesh_creater = MeshCreater();
mesh_creater.RectangleMesh(60,120,point2=(365.76,365.76))
mesh = mesh_creater.get_mesh()
bndry = mesh_creater.get_facet()

n = FacetNormal(mesh)


num_cell = mesh.num_cells()
print("Total no. of cell: ", num_cell)
KSpace = FunctionSpace(mesh,'DG', 0)
phi = Function(KSpace)
df = pd.read_csv("data/spe10_60x120_xy_tri.csv")
phi.vector()[:] = df.phi[:]
phi.rename("phi","poro")
...
# File("results_spe10/phi.pvd") << phi
# File("results_spe10/k.pvd")  << k_




# # create data only once and comment it
# def createData(mesh):
#     num_cell = mesh.num_cells()
#     KSpace = FunctionSpace(mesh,'DG', 0)
#     phi = Function(KSpace)
...
# print total degree of freedom
print("Total dof: ", len(W.dofmap().dofs()))
...
def getBCs():
    ...
    # bottom boundary
    bcs.append(DirichletBC(W.sub(0).sub(1), 0.0, bndry,4)) # No normal displacement for solid on bottom
    bcs.append(DirichletBC(W.sub(1), (0.0,0.0),  bndry,4))
...
L0 = inner(sigma(umid), sym(grad(v)))*dx  -  alpha*pmid*div(v)*dx \
    - inner(f, v)*dx - inner(t, v)*dss(3) # + alpha*inner(pbc*n,v)*dss(1)
...
# Output file
ufile = XDMFFile("results_spe10/u.xdmf")
pfile = XDMFFile("results_spe10/p.xdmf")
