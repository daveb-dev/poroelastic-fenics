...
"""
heterogenous isotropic problem
...
mesh_creater.RectangleMesh(16,16,point2=(1,1))
...
File("results/phi.pvd") << phi
File("results/k.pvd")  << k_
...
def getBCs():
    ...
    bcs.append(DirichletBC(W.sub(1), (0.0,0.0), bndry, 1)) # no flux

    
    # Right boundary
    bcs.append(DirichletBC(W.sub(0).sub(0), 0.0, bndry,2)) # No normal displacement for solid on right
    bcs.append(DirichletBC(W.sub(1), (0.0,0.0), bndry,2))  # no flux
    ...
    bcs.append(DirichletBC(W.sub(1), (0.0,0.0),  bndry,4)) # no flux
...
# Darcy velocity
L1 =(inner(Kinv*qmid, z) - pmid*div(z))*dx #- inner(z, pbc*n)*dss(3) # pressure bc term
